...
# TAB 10: EVE AI
with tabs[9]:
    st.header("🤖 EVE AI Assistant")
    
    st.markdown("""
    <div style='background: rgba(157, 0, 255, 0.1); padding: 20px; border-radius: 10px; border: 1px solid rgba(157, 0, 255, 0.3); text-align: center;'>
        <h2>👋 Hello, I'm EVE</h2>
        <p>Enhanced Virtual Entity - Your AI assistant</p>
        <p><strong>System Code:</strong> CEC_WAM_HEI_EVE_7A2F-9C4B</p>
        <p><strong>Owner:</strong> Twan | <strong>Access Level:</strong> Full</p>
        <p><strong>Personality:</strong> Professional, Helpful, Intelligent, Learning</p>
        <p style='color: #00FF88;'><strong>Status:</strong> ✓ Online and Ready</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with cap_col2:
        st.markdown("""
        <div style='background: rgba(157, 0, 255, 0.1); padding: 15px; border-radius: 10px; border: 1px solid rgba(157, 0, 255, 0.3);'>
            <h4>🔧 System Access</h4>
            <ul style='text-align: left;'>
    ...
    if 'messages' not in st.session_state:
        st.session_state.messages = [
            {"role": "assistant", "content": "Hello! I'm EVE, your Enhanced Virtual Entity. How can I assist you today?"}
        ]
    ...
    # Chat input
    if prompt := st.chat_input("Ask EVE anything..."):
        ...
        with st.chat_message("assistant"):
            # Simple response system (can be enhanced with OpenAI API)
            response = f"I received your message: '{prompt}'. To enable full AI capabilities, please configure the OpenAI API key in your environment variables."
            st.write(response)
    ...
    with col1:
        if st.button("🎤 Voice Input", use_container_width=True):
            st.info("Voice input requires microphone access. Configure SpeechRecognition API.")
    
    with col2:
        if st.button("🔊 Voice Output", use_container_width=True):
            st.info("Voice synthesis requires ElevenLabs API configuration.")
